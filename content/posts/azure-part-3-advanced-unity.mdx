...
- [Part 1: Complete Guide to Medallion Architecture](/blog/azure-part-1-medallion-architecture) - Building the foundation with Azure Data Factory and Databricks
- [Part 2: Unity Catalog Integration & Advanced Features](/blog/azure-part-1-medallion-architecture) - Adding enterprise-grade governance and monitoring

## Introduction

In [Part 1](/blog/azure-part-1-medallion-architecture), we established our medallion architecture foundation with Azure Data Factory and Databricks. [Part 2](/blog/azure-part-1-medallion-architecture) introduced Unity Catalog integration for data governance. Now, we’re taking our pipeline to the next level by implementing sophisticated table management strategies that optimize performance, cost, and governance.
...
WITH (STORAGE CREDENTIAL your_storage_credential) COMMENT 'Unity Catalog default managed storage location';
```
...
def manage_table_type_with_native_feature(table_name, should_be_managed):
    ...
    try:
        ...
        table_info = get_table_info(table_name) if not table_info['exists']:
            print(f" Table {table_name} does not exist") return False
        ...
        if should_be_managed and current_type == "EXTERNAL":
            # Convert to MANAGED
            print(f" Converting EXTERNAL → MANAGED...") spark.sql(f"ALTER TABLE {table_name} SET MANAGED") print(f" Successfully converted to MANAGED table") return True
            
        elif not should_be_managed and current_type == "MANAGED":
            # Convert to EXTERNAL
            print(f" Converting MANAGED → EXTERNAL...") spark.sql(f"ALTER TABLE {table_name} UNSET MANAGED") print(f" Successfully converted to EXTERNAL table") return True
            
        else:
            print(f" Table is already in the desired state: {current_type}") return True
            
    except Exception as e:
        error_msg = str(e) if "EXTERNAL_LOCATION_DOES_NOT_EXIST" in error_msg:
            print(" Unity Catalog storage location not configured!") print(" Create 'unity-catalog-storage' external location first") print(" See prerequisites section for setup instructions") return False
```
...
def read_table_data(table_name, table_type, silver_path):
    ...
    if table_type == "MANAGED":
        ...
        print("Reading from Unity Catalog managed table...") df = spark.table(table_name) else:
        # EXTERNAL tables can still be accessed via path
        print("Reading from silver path...") df = spark.read.format("delta").load(silver_path) return df
```
...
def determine_table_type(table_metadata):
    ...
    # Performance factors (favor managed) if table_metadata['query_frequency'] > 100:  # queries/day
        score += 3
    ...
    # Governance factors (favor managed) if table_metadata['requires_row_level_security']:
        score += 3
    if table_metadata['requires_column_masking']:
        score += 2
        
    # Flexibility factors (favor external) if table_metadata['external_consumers'] > 0:
        score -= 3
