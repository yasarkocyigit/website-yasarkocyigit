...
Today, I’ll walk you through building a complete **Medallion Architecture** data pipeline using Azure services. By the end of this article, you’ll have a production-ready solution that can process terabytes of data with minimal maintenance.
    ...
    properties = @{
        activities = @(
            ...
            @{
                name = "Copy to Bronze"
                type = "Copy"
                dependsOn = @(@{ activity = "Build Source Query"; dependencyConditions = @("Succeeded") })
                typeProperties = @{
                    source = @{
    ...
    properties = @{
        activities = @(
            ...
            @{
                ...
                type = "ForEach"
                dependsOn = @(@{ activity = "Get Tables for Silver"; dependencyConditions = @("Succeeded") })
                typeProperties = @{
    ...
    properties = @{
        activities = @(
            ...
            @{
                ...
                type = "Lookup"
                dependsOn = @(@{ activity = "Set Processing Date"; dependencyConditions = @("Succeeded") })
                typeProperties = @{
                    source = @{
                        type = "AzureSqlSource"
            ...
            @{
                ...
                type = "ExecutePipeline"
                dependsOn = @(@{ activity = "ForEach Table"; dependencyConditions = @("Succeeded") })
                typeProperties = @{
...
primary_keys = dbutils.widgets.get("primary_keys").split(",") if dbutils.widgets.get("primary_keys") else []
...
if load_type == "Full":
    ...
    if DeltaTable.isDeltaTable(spark, silver_path):
        ...
        merge_condition = " AND ".join([f"source.{pk.strip()} = target.{pk.strip()}" for pk in primary_keys])
